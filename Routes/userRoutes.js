...
router.post("/api/productsavedata", async (req, res) => {
    ...
    try {
      ...
      res.json({ msg: "Data inserted", success: true, data: savedProduct });
    } catch (e) {
      res.status(500).json({ msg: e.message, success: false });
    }
    
});
//api to get all products using async and await
router.get("/api/products", async (req, res) => {
    try{
      const products = await Product.find();
      res.status(200).json({msg: "data fetched successfully", success: true, products});
    } catch (err) {
      res.status(500).json({msg: err.message, success: false});
    }
});
